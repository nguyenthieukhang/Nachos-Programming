...
// This function will read the file for us and return the openFileID,
// which is an index in the file table.
...
int FileTable::Seek(int position, int id)
{
    ...
    if (files[id] == NULL)
        return -1;
    if (position == -1)
        position = files[id]->Length();
    if (position > files[id]->Length())
        return -1;
...
int FileTable::getSize()
{
    return fileCount;
}

int FileTable::getFileDescriptor(OpenFileId id)
{
    if (id < 2 || id >= MAX_NUMBER_OF_FILES)
        return -1;
    if (files[id] == NULL)
        return -1;
    return files[id]->getFileDescriptor();
}